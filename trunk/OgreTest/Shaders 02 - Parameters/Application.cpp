...
void Application::createScene()
{
	...
	m_entity->setMaterialName("shader/gradient");

	color = Vector4(0.5f, 0.5f, 0, 1);
	radius = Vector4(3,0,0,0);
	SubEntity* m_subentity = m_entity->getSubEntity(0);
	m_subentity->setCustomParameter(0,color);//color
	m_subentity->setCustomParameter(1,radius);//radius
...
{
	Entity* m_entity;
	m_entity = m_sceneManager->getEntity("RZR-002");
	SubEntity* m_subentity = m_entity->getSubEntity(0);
	...
	m_subentity->setCustomParameter(0,color);
	m_subentity->setCustomParameter(1,radius);
}
